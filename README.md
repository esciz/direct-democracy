...
`sources:refresh:daily` is the broader civic refresh. `.github/workflows/civic-data-production.yml` provides six-hour meeting collection, a daily broader refresh, persistent private checkpoints, a frozen public-data handoff to a separate publication job, and live release verification. `.github/workflows/application-build.yml` runs application regressions, typechecking and builds independently on code changes; scheduled collection does not build the app. The older DataOps workflow remains a manual recovery runner. See [launch operations](docs/launch-operations.md) for configuration, rollout and recovery; local collection alone does not publish the site.
