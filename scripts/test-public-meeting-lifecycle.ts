...
const retainedCanonical = fixture({ meeting_alias_ids: ["legacy-id"], meeting_date: "2026-06-18T16:00:00Z", title: "Official June meeting" });
const resurrectedAlias = fixture({ id: "legacy-id", meeting_date: "2026-08-06T16:00:00Z", title: "Old manifest date", minutes_url: "https://example.gov/june-minutes.pdf",
  source_urls: ["https://example.gov/june-minutes.pdf"], source_local_paths: ["retained-minutes.pdf"], document_hashes: ["retained-hash"] });
const unrelatedSource = fixture({ id: "other-source", public_body_id: "other-board" });
const scopedReplay = mergeMeetingHistory([resurrectedAlias, retainedCanonical], [unrelatedSource]);
assert.equal(scopedReplay.length, 2, "A scoped refresh must also consume retained aliases outside the refreshed source");
const canonicalAfterReplay = scopedReplay.find((meeting) => meeting.id === retainedCanonical.id)!;
assert.equal(canonicalAfterReplay.meeting_date, retainedCanonical.meeting_date, "A reappearing alias cannot replace the canonical attendance date");
assert.equal(canonicalAfterReplay.title, retainedCanonical.title);
assert.equal(canonicalAfterReplay.minutes_url, resurrectedAlias.minutes_url);
assert.ok(canonicalAfterReplay.source_local_paths?.includes("retained-minutes.pdf"));
assert.ok(canonicalAfterReplay.document_hashes.includes("retained-hash"));
assert.ok(canonicalAfterReplay.meeting_alias_ids?.includes(resurrectedAlias.id));
assert.deepEqual(mergeMeetingHistory(scopedReplay, []), scopedReplay, "Alias cleanup is idempotent and does not freshen evidence");
assert.equal(mergeMeetingHistory([retainedCanonical], [resurrectedAlias]).length, 1, "Refreshing the old provider ID cannot resurrect a declared alias");
assert.equal(mergeMeetingHistory([retainedCanonical, resurrectedAlias], []).length, 1, "A failed source discovery still applies retained identity evidence");
const chained = mergeMeetingHistory([retainedCanonical, { ...resurrectedAlias, meeting_alias_ids: ["older-id"] }, fixture({ id: "older-id" })], []);
assert.equal(chained.length, 1);
assert.deepEqual(new Set(chained[0].meeting_alias_ids), new Set(["legacy-id", "older-id"]), "An unambiguous alias chain retains every historical route");
const secondOwner = fixture({ id: "different-canonical", meeting_alias_ids: ["legacy-id"] });
assert.equal(mergeMeetingHistory([retainedCanonical, secondOwner, resurrectedAlias], []).length, 3, "Conflicting alias owners cannot be guessed away");
assert.equal(mergeMeetingHistory([retainedCanonical, secondOwner, resurrectedAlias], [retainedCanonical]).length, 3, "Refreshing an owner cannot consume an alias claimed by another owner");
assert.equal(mergeMeetingHistory([retainedCanonical, { ...resurrectedAlias, meeting_alias_ids: [retainedCanonical.id] }], []).length, 2, "Cyclic alias claims remain reviewable");
assert.equal(mergeMeetingHistory([retainedCanonical, { ...resurrectedAlias, meeting_alias_ids: [retainedCanonical.id] }], [retainedCanonical]).length, 2, "Refreshing one member cannot consume a cyclic alias");
assert.equal(mergeMeetingHistory([resurrectedAlias], [retainedCanonical, secondOwner]).length, 3, "Competing incoming claims must be considered together before consuming an alias");
assert.equal(mergeMeetingHistory([fixture({ id: "ccsd-subcommittee" }), fixture({ id: "ivgid-subcommittee" })], []).length, 2, "Shared body/day and documents do not establish a new alias");
