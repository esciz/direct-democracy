...
The collection job compacts runtime data, saves its private checkpoint, applies reporting policy, regenerates integrity findings, and runs civic release regressions before freezing the release manifest. It hands only the manifest and allowlisted public runtime files to a separate publication job in the same workflow. That job checks the source commit and every file hash before publishing a versioned release. Retrying publication reuses the frozen candidate without repeating collection; a separate source-coverage job preserves collection failures. The `Application Build` workflow runs application/source regressions, typechecking, the production build and bundle audit on pushes and pull requests, without production credentials or source collection. Content-addressed objects are verified before the latest manifest changes. Runtime publication rejects malformed or missing required datasets, critical integrity problems and material unexplained record loss. Incomplete source coverage stays visible in the manifest and audits; publication does not certify statewide completeness. A failed collection retains its failure status even when a usable release is published.

Vercel's build command restores the selected validated runtime release before `npm run build`. Set `CIVIC_DATA_RELEASE_ENABLED=true` for this path; ordinary local builds remain independent of cloud storage. The collection and publication jobs explicitly disable release restore. They do not run an application build or change runtime bytes after preparing the candidate; Vercel builds the published release separately. Large PDF/text/OCR caches and the full accountability graph remain on the worker and outside web functions. `npm run dataops:runtime:compact` writes the exact community accountability summaries consumed by the UI and removes JSON whitespace before snapshotting; it preserves every public record and keeps the full graph intact. The event bundle audit also enforces a 250 MiB packaging budget.

The deploy hook rebuilds `main` after publication. Collection, publication/deployment and application-build failures have separate job/workflow names. On September 9, publication was failing with `civic_snapshot_changed_during_upload`: the old workflow prepared hashes before `npm run build`, whose reporting-policy prebuild rewrote runtime files. The frozen handoff removes that mutation window, and publication now reports changed filenames before any upload. Never work around this by disabling hash verification. The workflow verifies that the public `/api/data-release` returns the expected hash, with up to 45 minutes for packaging/deployment. It fails if publication never reaches the public site. A Vercel deployment can be healthy while serving an older release; compare identities, not just HTTP 200.
...
# For manual publication, run all mutating tests/build steps BEFORE prepare.
# Prepare again after any reporting-policy or runtime changes.
npm run dataops:release:prepare
# Publish the validated frozen runtime.
...
Migrated identity accounts may have no linked public `User` record. Optional profile reads return empty content in that case so the shared shell and MFA pages can load; profile writes still require the linked record. A missing public profile must never grant admin access or replace the durable session and MFA checks. Regression coverage: `node --import tsx scripts/test-durable-profile-content.ts` and `npm run auth:sessions:test`.

MFA recovery codes are verified independently of the encrypted TOTP secret. A lost or changed encryption key must not prevent use of an unused recovery code. Authenticator decryption failures show a recovery prompt and do not count as incorrect codes; account eligibility, rate limits, session rotation and atomic single-use consumption still apply. Regression coverage: `node --import tsx scripts/test-mfa-recovery.ts`.

Admin human-review decisions use the `HumanReviewDecision` database table, keyed by review category and item ID, with an atomic audit event for each save. Apply `prisma/migrations/20260907223000_durable_human_review/migration.sql` before deploying this flow. The historical generated review file is a read-only fallback, never a production write target. Database failure disables saves and shows an explicit message. Completed decisions leave the default Open view; Completed and All views retain them, and pagination exposes the entire queue. These decisions track review progress without promoting changes to public source records. Check with `node --import tsx scripts/test-human-review.ts`.
